...
subarray whose sum >= target. If there is no such subarray, return 0 instead.
Eg:
...
public class MinSizeSubarraySum {
    ...
    public static int minSubArrayLenApproach2(int target, int[] nums) {
        int leftPtr = 0;
        int sum = 0;
        int minLength = Integer.MAX_VALUE;
        ...
        for(int rightPtr=0; rightPtr<nums.length; rightPtr++){
                ...
                if(sum>=target){
                    ...
                    sum -= nums[leftPtr]; // Remove the element at leftPtr from the current subarray
                    leftPtr++; //Move leftPtr to the right
            }
        }
    ...
    public static void main(String []args){
        ...
        System.out.println("Minimum Size Subarray Sum"+minSubArrayLenApproach2(target, nums));
