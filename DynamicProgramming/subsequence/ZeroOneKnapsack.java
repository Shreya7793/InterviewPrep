...
public class ZeroOneKnapsack {
    static int knapsackWithRecursion(int ind, int targetWeight, int []weight, int []val){
        ...
        int pick = Integer.MIN_VALUE; //since we have to maximise pick value. Therefore take the min value.
    ...
    //SC: recursion stack space(O(N)) + a 2D array ( O(N*W)).
    //TC: N*W states therefore at max ‘N*W’ new problems will be solved
    static int knapsackWithMemoisation(int ind, int targetWeight, int []weight, int []val, int [][]dp){
        ...
        if(weight[ind]<=targetWeight)
            pick= val[ind] + knapsackWithMemoisation(ind-1, targetWeight-weight[ind], weight, val,dp);
